...
export class TasksService {
  ...
  async updateTasks(
    ...
    const promises = tasks.map(async (task) => {
      ...
      // @ts-ignore
      const updatedTask = this.todoist.items
        .get()
        .find((item: any) => item.id === task.task_id);
    ...
    return relevantTasks.filter((task: any) => {
      ...
      if (startDate || endDate) {
        const taskDate = task.due?.date
          ? new Date(task.due.date + "T00:00:00Z")
          : null;
        if (taskDate) {
